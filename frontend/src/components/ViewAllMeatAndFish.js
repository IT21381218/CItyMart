...
const ViewAllMeatAndFish = () => {
  const [meatAndFish, setMeatAndFish] = useState(null);
  ...
  useEffect(() => {
    const fetchMeatAndFish = async () => {
      try {
        ...
        const data = response.data.filter(product => product.category_type === 'meatandfish');
        setMeatAndFish(data);
      } catch (error) {
        console.error('Error fetching meat and fish:', error);
      }
    };
  ...
  const meatAndFishDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/product/delete/${id}`);
      setMeatAndFish(meatAndFish.filter(item => item.id !== id));
  ...
  const generatePDF = () => (
    <Document>
      <Page style={styles.page}>
        <Text style={styles.header}>List of Meat and Fish</Text>
        {meatAndFish && meatAndFish.map((item, index) => (
  ...
  return (
    <div style={styles.container}>
      <div style={styles.searchContainer}>
        ...
        />
        <Link to='/addProduct' style={styles.addButton}>+ Add New Meat and Fish</Link>
        <PDFDownloadLink document={generatePDF()} fileName="meat_and_fish.pdf" style={styles.downloadButton}>
      ...
      <table style={styles.table}>
        <thead style={styles.tableHeader}>
          <tr>
            ...
            <th scope="col" style={styles.tableCell}>Meat / Fish Name</th>
        ...
        <tbody>
          {meatAndFish && meatAndFish.filter((item) => (
            ...
            <tr key={item._id}>
              ...
              <td style={{ ...styles.tableCell, ...styles.actionButtons }}>
                ...
                <button className="btn btn-danger" onClick={() => meatAndFishDelete(item._id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default ViewAllMeatAndFish;
