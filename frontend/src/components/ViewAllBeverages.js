...
const ViewAllBeverages = () => {
  ...
  useEffect(() => {
    const fetchBeverages = async () => {
      try {
        ...
        const data = response.data.filter(product => product.category_type === 'beverages');
        setBeverages(data);
      } catch (error) {
  ...
  const beverageDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/product/delete/${id}`);
      setBeverages(beverages.filter(item => item.id !== id));
  ...
  const generatePDF = () => (
    <Document>
      <Page style={styles.page}>
        <Text style={styles.header}>List of Beverages</Text>
  ...
  return (
    <div style={styles.container}>
      <div style={styles.searchContainer}>
        ...
        />
        <Link to='/addProduct' style={styles.addButton}>+ Add New Beverage</Link>
        <PDFDownloadLink document={generatePDF()} fileName="beverages.pdf" style={styles.downloadButton}>
      ...
      <table style={styles.table}>
        <thead style={styles.tableHeader}>
          <tr>
            ...
            <th scope="col" style={styles.tableCell}>Beverage Name</th>
          ...
          {beverages && beverages.filter((item) => (
            ...
            <tr key={item._id}>
              ...
              <td style={{ ...styles.tableCell, ...styles.actionButtons }}>
                ...
                <button className="btn btn-danger" onClick={() => beverageDelete(item._id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default ViewAllBeverages;
